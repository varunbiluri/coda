...
class TestLiteLLMClient:
    ...
    def test_litellm_planner_spec_generation(self, mock_completion):
        ...
        mock_response.choices[0].message.content = (
            '{"tasks": [{"id": "test", "description": "Test task"}], "context": "Test context", "estimated_changes": ["Change 1"]}'
        )
    ...
    def test_litellm_code_diff_generation(self, mock_completion):
        ...
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = (
            '{"diff": "--- a/file.py", "commit_message": "Test commit", "explanation": "Test explanation"}'
        )
        mock_completion.return_value = mock_response
...
class TestCreateLLMClient:
    ...
    def test_create_litellm_client_openai(self):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            client = create_llm_client(
                use_mock=False, model="gpt-3.5-turbo", provider="openai"
            )
    ...
    def test_create_litellm_client_anthropic(self):
        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key"}):
            client = create_llm_client(
                use_mock=False, model="claude-3-sonnet", provider="anthropic"
            )
            assert isinstance(client, LiteLLMClient)
            assert client.provider == "anthropic"
